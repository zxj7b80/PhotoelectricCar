...
unsigned char pll_init(unsigned char clk_option, unsigned char crystal_val)
{
    ...
    if (clk_option > 8) {return 0;}   //如果没有选择可用的选项则返回0
    if (crystal_val > 16) {return 1;} // 如果如果可用的晶体选项不可用则返回1
    ...
    #if (defined(K60_CLK))          //使用外部的50M晶振 
  //MCG_C5 = MCG_C5_PRDIV(0x18);    //当然需要25分频，在本函数里没有XTAL50，所以搞了个K60_CLK，表示用了外部的50M
  //MCG_C5 = MCG_C5_PRDIV(0x13);    //20分频，50M晶振，2.5M  
  //MCG_C5 = MCG_C5_PRDIV(12);      //13分频  3.846M  150M时钟频率使用
  //MCG_C5 = MCG_C5_PRDIV(3);       //外部使用的是8M晶振的状况   1:2分频  3:4分频
    MCG_C5 = MCG_C5_PRDIV(0x0e);    //180M
  // MCG_C5 = MCG_C5_PRDIV(0x0C);    //207M
  //MCG_C5 = MCG_C5_PRDIV(0x0b);    //225M
    #else
    ...
    case 4:
      ...
      break;
    case 5:
      set_sys_dividers(0,2,2,7);  //core=180M, bus=60M, FlexBus=60M, Flash Clk=30M
      MCG_C6 = MCG_C6_PLLS_MASK | MCG_C6_VDIV(30); //VDIV = 15 (3.846x39) 150MHz
      pll_freq = 180;
      break;
    case 6:
      set_sys_dividers(0,2,2,7);  //core=180M, bus=60M, FlexBus=60M, Flash Clk=30M
      ...
      break;
    case 7:
      set_sys_dividers(0,2,2,8);  //core=180M, bus=60M, FlexBus=60M, Flash Clk=30M
      MCG_C6 = MCG_C6_PLLS_MASK | MCG_C6_VDIV(30); //VDIV = 15 (3.846x39) 150MHz
      pll_freq = 225;
      break;
    }
